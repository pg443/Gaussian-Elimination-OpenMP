...
void gauss_eliminate_using_omp(Matrix, int, int);
...
int main(int argc, char **argv)
{
    ...
    gettimeofday(&start, NULL);
    gauss_eliminate_using_omp(U_mt, matrix_size, num_threads);
    gettimeofday(&stop, NULL);
    fprintf(stderr, "CPU run time = %f s\n", (float)(stop.tv_sec - start.tv_sec\
                + (stop.tv_usec - start.tv_usec) / (float)1000000));
...
void gauss_eliminate_using_omp(Matrix U, int num_elements, int num_threads)
{
    
    int i,j,k; 
    float *elements = U.elements; 
   #pragma omp parallel num_threads(num_threads) private(i,j,k) shared(num_elements, num_threads, elements) 
   {
        for (k = 0; k < num_elements; k++){
           #pragma omp for schedule(guided)
            for (j = (k + 1); j < num_elements; j++) {   /* Reduce the current row. */
                if (elements[num_elements * k + k] == 0) {
                    fprintf(stderr, "Numerical instability. The principal diagonal element is zero.\n");
                    exit(EXIT_FAILURE);
                }      
                elements[num_elements * k + j] = (float)(elements[num_elements * k + j] / elements[num_elements * k + k]);	/* Division step */
            }
           
            elements[num_elements * k + k] = 1;	/* Set the principal diagonal entry in U to 1 */ 
            #pragma omp for schedule(guided)
            for (i = (k + 1); i < num_elements; i++) {
                for (j = (k + 1); j < num_elements; j++)
                    elements[num_elements * i + j] = elements[num_elements * i + j] - (elements[num_elements * i + k] * elements[num_elements * k + j]);	/* Elimination step */
                
                elements[num_elements * i + k] = 0;
            }
        //    #pragma omp barrier
        }
    }
